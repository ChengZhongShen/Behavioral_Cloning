...
# model_3d_yuv: fix the bug in nvaidia2, the FC has unecessary layer.
...
def load_images(images, steerings, path, sample_rate, sub_read=False, sample_balance=False):
    '''
...
def load_sides_images(images, steerings, path, sample_rate, offset, sub_read=False, sample_balance=False):
    '''
    ...
    for indx, line in tqdm(enumerate(lines)):
        ...
        image_left = process_image(image_path_left)

        image_right = process_image(image_path_right)
...
def model_nvidia2(X_train, y_train, save_name, color='YUV', epoch=10):
    ...
    # FC1
    model.add(Flatten())
    model.add(Dense(1164)) # uneccesary layer, in the paper there is no such layer. fixed in nvida2a
    model.add(Dropout(0.5))
    # FC2
    model.add(Dense(100))
    model.add(Dropout(0.5))
...
def model_nvidia2a(X_train, y_train, save_name, color='YUV', epoch=10):
    '''
    2018/11/27 rewrite the nvidia model according the papaer. (add three color choice, RGB/HSV/YUV)
    Trid tensorflow 1.8.0, but the speed of Workspace is not acceptable.
    the name should be *.h5
...
def train_model(data_path, sample_rate=1, data_sub_read=False, data_sample_balance=False, sides_image=True, sides_image_offset=0.22, data_augmentation=1, 
    ...
    for path in data_path:
        # load center image
        load_images(images, steerings, path=path, sample_rate=sample_rate, sub_read=data_sub_read, sample_balance=data_sample_balance)        
        # load sides image if the flag 'sides_image' is True
        if sides_image:
            load_sides_images(images, steerings, path=path, sample_rate=sample_rate,sub_read=data_sub_read, sample_balance=data_sample_balance, offset=sides_image_offset)   
    ...
    if model == 'nvidia':
        model_name="m-{}_s-{}_lr-{}-{}_db{}_{}_aug{}_ep{}.h5".format(
                        model,sample_rate, sides_image,sides_image_offset,data_sample_balance,input_color,data_augmentation,train_epoch)
        ...
        print(model_name, 'will be training and saved!!')
        print()
        model_nvidia2a(X_train, y_train, model_name, color=input_color, epoch=train_epoch)
...
if __name__ == '__main__':
    ...
    train_model(data_path=('./data/', './data_t2_2/'), sample_rate=3, data_sub_read=50, data_sample_balance=0.01, sides_image=True, sides_image_offset=0.22, data_augmentation=1, 
                model='nvidia2', input_color='YUV', train_epoch=1)