...
def load_images(images, steerings, path, sub_read=False, sample_balance=False):
    '''
...
def load_sides_images(images, steerings, path, offset, sub_read=False, sample_balance=False):
    '''
...
def model_nvidia2(X_train, y_train, save_name, color='RGB', epoch=10):
    ...
    if color == "RGB":
        ...
        model.add(Lambda(preprocess_yiq, input_shape=(160,320,3))) 
...
def train_model(data_path, data_sub_read=False, data_sample_balance=False, sides_image=True, sides_image_offset=0.22, data_augmentation=1, 
    ...
    for path in data_path:
        # load center image
        load_images(images, steerings, path=path, sub_read=data_sub_read, sample_balance=data_sample_balance)        
        # load sides image if the flag 'sides_image' is True
        if sides_image:
            load_sides_images(images, steerings, path=path, sub_read=data_sub_read, sample_balance=data_sample_balance, offset=sides_image_offset)   
...
if __name__ == '__main__':
    ...
    train_model(data_path=('./data/', './data_t2_2/'), data_sub_read=False, data_sample_balance=0.01, sides_image=True, sides_image_offset=0.22, data_augmentation=1, 
